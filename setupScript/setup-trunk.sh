...
bash -c "sudo apt-get -qy install maven2 subversion make g++ g++-multilib; svn co https://svn.apache.org/repos/asf/accumulo/trunk /tmp/accumulo; cd /tmp/accumulo/server/src/main/c++/nativeMap; make; cd /tmp/accumulo/; mvn package -P assemble" &
accumuloInstall=$!
...
tar xzf /tmp/accumulo/assemble/target/accumulo-1.5.0-SNAPSHOT-dist.tar.gz -C /tmp
sudo mkdir /usr/lib/accumulo
sudo mv /tmp/accumulo-1.5.0-SNAPSHOT/* /usr/lib/accumulo/
...
cd /usr/lib/accumulo/assemble/platform/debian
sudo sh stand-alone-init.sh

# Start ALL the Accumulo processes
accumuloPids=""
for accumulo in /etc/init.d/accumulo*; do
  sudo $accumulo start
  accumuloPids="$accumuloPids $!"
done
